...
def main():
    ...
    columns = ['const_1_cp', 'const_1_nodes_searched', 'const_1_cp_multipv_variability']
    for column in columns:
        if column not in df.columns:
            df[column] = np.nan

    unprocessed_df = df[pd.isna(df['const_1_cp'])]

    total_rows = len(unprocessed_df)
    print(f"Total positions to analyze: {total_rows}")

    for index, row in unprocessed_df.iterrows():
        fen_1 = row['constellation_fen_1']
        player_made = row['player_made']

        try:
            with chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH) as engine:
                cp_score, nodes_searched, score_variability, error_occurred = analyze_fen(engine, fen_1, player_made)
                
                if not error_occurred:
                    # Update the original DataFrame with analysis results
                    df.at[index, 'const_1_cp'] = cp_score
                    df.at[index, 'const_1_nodes_searched'] = nodes_searched
                    df.at[index, 'const_1_cp_multipv_variability'] = score_variability
                else:
                    df.at[index, 'const_1_cp'] = 'NA'
        except chess.engine.EngineTerminatedError as e:
            print(f"Engine terminated unexpectedly for FEN {fen_1}: {e}")
            df.at[index, 'const_1_cp'] = 'NA'
